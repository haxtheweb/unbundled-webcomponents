 ...
 *   implementation.
 */var DomIfBase=/*#__PURE__*/function(_PolymerElement){babelHelpers.inherits(DomIfBase,_PolymerElement);babelHelpers.createClass(DomIfBase,null,[{key:"is",// Not needed to find template; can be removed once the analyzer
...
get:function get(){return"dom-if"}},{key:"template",get:function get(){return null}},{key:"properties",get:function get(){return{/**
       ...
       * element.
       */notifyDomChange:{type:Boolean}}}}]);function DomIfBase(){var _this;babelHelpers.classCallCheck(this,DomIfBase);_this=babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(DomIfBase).call(this));_this.__renderDebouncer=null;_this._lastIf=!1;_this.__hideTemplateChildren__=!1;/** @type {!HTMLTemplateElement|undefined} */_this.__template;/** @type {!TemplateInfo|undefined} */_this._templateInfo;return _this}babelHelpers.createClass(DomIfBase,[{key:"__debounceRender",value:function __debounceRender(){var _this2=this;// Render is async for 2 reasons:
...
this.__renderDebouncer=_debounce.Debouncer.debounce(this.__renderDebouncer,_async.microTask,function(){return _this2.__render()});(0,_flush.enqueueDebouncer)(this.__renderDebouncer)}/**
   * @override
   * @return {void}
   */},{key:"disconnectedCallback",value:function disconnectedCallback(){babelHelpers.get(babelHelpers.getPrototypeOf(DomIfBase.prototype),"disconnectedCallback",this).call(this);var parent=(0,_wrap.wrap)(this).parentNode;if(!parent||parent.nodeType==Node.DOCUMENT_FRAGMENT_NODE&&!(0,_wrap.wrap)(parent).host){this.__teardownInstance()}}/**
   * @override
   * @return {void}
   */},{key:"connectedCallback",value:function connectedCallback(){babelHelpers.get(babelHelpers.getPrototypeOf(DomIfBase.prototype),"connectedCallback",this).call(this);if(!(0,_hideTemplateControls.hideElementsGlobally)()){this.style.display="none"}if(this.if){this.__debounceRender()}}/**
   ...
   */},{key:"__ensureTemplate",value:function __ensureTemplate(){var _this3=this;if(!this.__template){// When `removeNestedTemplates` is true, the "template" is the element
// itself, which has been given a `_templateInfo` property
var thisAsTemplate=/** @type {!HTMLTemplateElement} */ /** @type {!HTMLElement} */this,template=thisAsTemplate._templateInfo?thisAsTemplate:/** @type {!HTMLTemplateElement} */(0,_wrap.wrap)(thisAsTemplate).querySelector("template");if(!template){// Wait until childList changes and template should be there by then
var observer=new MutationObserver(function(){if((0,_wrap.wrap)(_this3).querySelector("template")){observer.disconnect();_this3.__render()}else{throw new Error("dom-if requires a <template> child")}});observer.observe(this,{childList:!0});return!1}this.__template=template}return!0}/**
   ...
   */},{key:"__ensureInstance",value:function __ensureInstance(){var parentNode=(0,_wrap.wrap)(this).parentNode;if(!this.__hasInstance()){// Guard against element being detached while render was queued
...
var children=this.__getInstanceNodes();if(children&&children.length){// Detect case where dom-if was re-attached in new position
var lastChild=(0,_wrap.wrap)(this).previousSibling;if(lastChild!==children[children.length-1]){for(var i=0,n;i<children.length&&(n=children[i]);i++){(0,_wrap.wrap)(parentNode).insertBefore(n,this)}}}}return!0}/**
   ...
   *
   * @return {void}
   */},{key:"render",value:function render(){(0,_flush.flush)()}/**
...
/**
   ...
   * @protected
   * @return {void}
   */},{key:"_showHideChildren",value:function _showHideChildren(){}/* eslint-enable valid-jsdoc */}]);return DomIfBase}(_polymerElement.PolymerElement),DomIfFast=/*#__PURE__*/function(_DomIfBase){babelHelpers.inherits(DomIfFast,_DomIfBase);function DomIfFast(){var _this4;babelHelpers.classCallCheck(this,DomIfFast);_this4=babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(DomIfFast).call(this));_this4.__instance=null;_this4.__syncInfo=null;return _this4}/**
   ...
   * @param {Node} parentNode The parent node to insert the instance into
   * @return {void}
   */},{key:"__createAndInsertInstance",value:function __createAndInsertInstance(parentNode){var _this5=this,host=this.__dataHost||this;if(_settings.strictTemplatePolicy){if(!this.__dataHost){throw new Error("strictTemplatePolicy: template owner not trusted")}}// Pre-bind and link the template into the effects system
...
templateInfo.runEffects=function(runEffects,changedProps,hasPaths){var syncInfo=_this5.__syncInfo;if(_this5.if){// Mix any props that changed while the `if` was false into `changedProps`
...
_this5.__syncInfo=null;_this5._showHideChildren();changedProps=Object.assign(syncInfo.changedProps,changedProps)}runEffects(changedProps,hasPaths)}else{// Accumulate any values changed while `if` was false, along with the
// runEffects method to sync them, so that we can replay them once `if`
// becomes true
if(_this5.__instance){if(!syncInfo){syncInfo=_this5.__syncInfo={runEffects:runEffects,changedProps:{}}}if(hasPaths){// Store root object of any paths; this will ensure direct bindings
...
// __invalidProps.
for(var p in changedProps){var rootProp=(0,_path.root)(p);syncInfo.changedProps[rootProp]=_this5.__dataHost[rootProp]}}else{Object.assign(syncInfo.changedProps,changedProps)}}}};// Stamp the template, and set its DocumentFragment to the "instance"
this.__instance=host._stampTemplate(/** @type {!HTMLTemplateElement} */this.__template,templateInfo);(0,_wrap.wrap)(parentNode).insertBefore(this.__instance,this)}/**
   ...
   * @protected
   * @suppress {visibility}
   */},{key:"_showHideChildren",value:function _showHideChildren(){var hidden=this.__hideTemplateChildren__||!this.if;if(this.__instance&&!!this.__instance.__hidden!==hidden){this.__instance.__hidden=hidden;(0,_templatize.showHideChildren)(hidden,this.__instance.templateInfo.childNodes)}if(!hidden){this.__syncHostProperties()}}}]);return DomIfFast}(DomIfBase),DomIfLegacy=/*#__PURE__*/function(_DomIfBase2){babelHelpers.inherits(DomIfLegacy,_DomIfBase2);function DomIfLegacy(){var _this6;babelHelpers.classCallCheck(this,DomIfLegacy);_this6=babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(DomIfLegacy).call(this));_this6.__ctor=null;_this6.__instance=null;_this6.__invalidProps=null;return _this6}/**
   * Implementation of abstract API needed by DomIfBase.
   *
   * @override
   * @return {boolean} True when an instance has been created.
...
if(parent){parent=(0,_wrap.wrap)(parent);for(var i=0,n;i<c$.length&&(n=c$[i]);i++){parent.removeChild(n)}}}this.__invalidProps=null;this.__instance=null}}/**
