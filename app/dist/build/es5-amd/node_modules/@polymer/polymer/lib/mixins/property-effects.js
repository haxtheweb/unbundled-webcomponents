...
var dedupeId=0,NOOP=[],TYPES={COMPUTE:"__computeEffects",REFLECT:"__reflectEffects",NOTIFY:"__notifyEffects",PROPAGATE:"__propagateEffects",OBSERVE:"__observeEffects",READ_ONLY:"__readOnly"},COMPUTE_INFO="__computeInfo",capitalAttributeRegex=/[A-Z]/,DataTrigger,DataEffect;//eslint-disable-line no-unused-vars
/**
 ...
 * @private
 */function ensureOwnEffectMap(model,type,cloneArrays){var effects=model[type];if(!effects){effects=model[type]={}}else if(!model.hasOwnProperty(type)){effects=model[type]=Object.create(model[type]);if(cloneArrays){for(var p in effects){for(var protoFx=effects[p],instFx=effects[p]=Array(protoFx.length),i=0;i<protoFx.length;i++){instFx[i]=protoFx[i]}}}}return effects}// -- effects ----------------------------------------------
/**
 ...
 * @private
 */function runEffects(inst,effects,props,oldProps,hasPaths,extraArgs){if(effects){var ran=!1,id=dedupeId++;for(var prop in props){// Inline `runEffectsForProperty` for perf.
var rootProperty=hasPaths?(0,_path.root)(prop):prop,fxs=effects[rootProperty];if(fxs){for(var i=0,l=fxs.length,fx;i<l&&(fx=fxs[i]);i++){if((!fx.info||fx.info.lastRun!==id)&&(!hasPaths||pathMatchesTrigger(prop,fx.trigger))){if(fx.info){fx.info.lastRun=id}fx.fn(inst,prop,props,oldProps,fx.info,hasPaths,extraArgs);ran=!0}}}}return ran}return!1}/**
...
(0,_wrap.wrap)(/** @type {!HTMLElement} */inst).dispatchEvent(new CustomEvent(eventName,{detail:detail}))}/**
 ...
 * @return {void}
 * @private
 */function runComputedEffects(inst,changedProps,oldProps,hasPaths){var computeEffects=inst[TYPES.COMPUTE];if(computeEffects){if(_settings.orderedComputed){// Runs computed effects in efficient order by keeping a topologically-
// sorted queue of compute effects to run, and inserting subsequently
// invalidated effects as they are run
dedupeId++;var order=getComputedOrder(inst),queue=[];for(var p in changedProps){enqueueEffectsFor(p,computeEffects,queue,order,hasPaths)}var info;while(info=queue.shift()){if(runComputedEffect(inst,"",changedProps,oldProps,info)){enqueueEffectsFor(info.methodInfo,computeEffects,queue,order,hasPaths)}}Object.assign(/** @type {!Object} */oldProps,inst.__dataOld);Object.assign(/** @type {!Object} */changedProps,inst.__dataPending);inst.__dataPending=null}else{// Original Polymer 2.x computed effects order, which continues running
...
for(var p in infoForComputed){var info=infoForComputed[p];// Be sure to add the method name itself in case of "dynamic functions"
total+=counts[p]=info.args.filter(function(a){return!a.literal}).length+(info.dynamicFn?1:0)}// Build list of ready properties (that aren't themselves computed)
for(var _p in computedDeps){if(!infoForComputed[_p]){ready.push(_p)}}return{counts:counts,ready:ready,total:total}}/**
 ...
 * @private
 */function createMethodEffect(model,sig,type,effectFn,methodInfo,dynamicFn){dynamicFn=sig.static||dynamicFn&&("object"!==babelHelpers.typeof(dynamicFn)||dynamicFn[sig.methodName]);for(var info={methodName:sig.methodName,args:sig.args,methodInfo:methodInfo,dynamicFn:dynamicFn},i=0,arg;i<sig.args.length&&(arg=sig.args[i]);i++){if(!arg.literal){model._addPropertyEffect(arg.rootProperty,type,{fn:effectFn,info:info,trigger:arg})}}if(dynamicFn){model._addPropertyEffect(sig.methodName,type,{fn:effectFn,info:info})}return info}/**
...
_this.__isPropertyEffectsClient=!0;/** @type {boolean} */_this.__dataClientsReady;/** @type {Array} */_this.__dataPendingClients;/** @type {Object} */_this.__dataToNotify;/** @type {Object} */_this.__dataLinkedPaths;/** @type {boolean} */_this.__dataHasPaths;/** @type {Object} */_this.__dataCompoundStorage;/** @type {Polymer_PropertyEffects} */_this.__dataHost;/** @type {!Object} */_this.__dataTemp;/** @type {boolean} */_this.__dataClientsInitialized;/** @type {!Object} */_this.__data;/** @type {!Object|null} */_this.__dataPending;/** @type {!Object} */_this.__dataOld;/** @type {Object} */_this.__computeEffects;/** @type {Object} */_this.__computeInfo;/** @type {Object} */_this.__reflectEffects;/** @type {Object} */_this.__notifyEffects;/** @type {Object} */_this.__propagateEffects;/** @type {Object} */_this.__observeEffects;/** @type {Object} */_this.__readOnly;/** @type {!TemplateInfo} */_this.__templateInfo;/** @type {boolean} */_this._overrideLegacyUndefined;return _this}babelHelpers.createClass(PropertyEffects,[{key:"_initializeProperties",/**
     * @override
     * @return {void}
...
// ----------------------------
     ...
     * @return {void}
     * @protected
     */},{key:"_propagatePropertyChanges",value:function _propagatePropertyChanges(changedProps,oldProps,hasPaths){if(this[TYPES.PROPAGATE]){runEffects(this,this[TYPES.PROPAGATE],changedProps,oldProps,hasPaths)}if(this.__templateInfo){this._runEffectsForTemplate(this.__templateInfo,changedProps,oldProps,hasPaths)}}},{key:"_runEffectsForTemplate",value:function _runEffectsForTemplate(templateInfo,changedProps,oldProps,hasPaths){var _this2=this,baseRunEffects=function baseRunEffects(changedProps,hasPaths){runEffects(_this2,templateInfo.propertyEffects,changedProps,oldProps,hasPaths,templateInfo.nodeList);for(var info=templateInfo.firstChild;info;info=info.nextSibling){_this2._runEffectsForTemplate(info,changedProps,oldProps,hasPaths)}};if(templateInfo.runEffects){templateInfo.runEffects(baseRunEffects,changedProps,hasPaths)}else{baseRunEffects(changedProps,hasPaths)}}/**
...
for(var nodes=templateInfo.childNodes,i=0,node;i<nodes.length;i++){node=nodes[i];(0,_wrap.wrap)((0,_wrap.wrap)(node).parentNode).removeChild(node)}}/**
...
var hostProps=nestedTemplateInfo.hostProps;if(_settings.fastDomIf&&isDomIf){// `fastDomIf` mode uses runtime-template stamping to add accessors/
...
if(!_settings.removeNestedTemplates){nodeInfo.parentInfo.noted=!0}}}else{var mode="{";for(var source in hostProps){var parts=[{mode:mode,source:source,dependencies:[source],hostProp:!0}];addBinding(this,templateInfo,nodeInfo,"property","_host_"+source,parts)}}return noted}/**
 ...
 *
 * @private
 */_exports.PropertyEffects=PropertyEffects;var hostStack=[]});